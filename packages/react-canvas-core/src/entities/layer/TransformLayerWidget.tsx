...
export class TransformLayerWidget extends React.Component<React.PropsWithChildren<TransformLayerWidgetProps>> {
	...
	render() {
		...
		if (this.props.layer.getOptions().isSvg) {
			return <S.SvgLayer  className='svg_layer'  style={this.getTransformStyle()}>{this.props.children}</S.SvgLayer>;
		}
			return <S.DivLayer className='node_layer' style={this.getTransformStyle()}>{this.props.children}</S.DivLayer>;
	}
}
